...
export default function AgreementPageClient({ bookingId, initialBooking, initialCar, initialAgreement }: AgreementPageClientProps) {
  ...
  const form = useForm<AgreementFormValues>({
    ...
    defaultValues: {
        agreementDate: initialAgreement?.agreementDate || format(new Date(), 'yyyy-MM-dd'),
        renterIdOrPassport: initialAgreement?.renterIdOrPassport || '',
        renterAddress: initialAgreement?.renterAddress || '',
        vehicleDetails: initialAgreement?.vehicleDetails || `${initialCar?.name} (${initialCar?.type})`,
        rentalStartDate: initialAgreement?.rentalStartDate || initialBooking.pickupDate,
        clientFullName: initialAgreement?.clientFullName || initialBooking.customerName,
        clientContactNumber: initialAgreement?.clientContactNumber || initialBooking.customerPhone,
        rentalDuration: initialAgreement?.rentalDuration || '',
        rentCostPerDayMonth: initialAgreement?.rentCostPerDayMonth || '',
        totalRentCost: initialAgreement?.totalRentCost || '',
        depositMoney: initialAgreement?.depositMoney || '',
        dailyKMLimit: initialAgreement?.dailyKMLimit || '',
        priceForAdditionalKM: initialAgreement?.priceForAdditionalKM || '',
        clientSignDate: initialAgreement?.clientSignDate || '',
        guarantorName: initialAgreement?.guarantorName || '',
        guarantorNIC: initialAgreement?.guarantorNIC || '',
        guarantorAddress: initialAgreement?.guarantorAddress || '',
        guarantorContact: initialAgreement?.guarantorContact || '',
        billDate: initialAgreement?.billDate || format(new Date(), 'yyyy-MM-dd'),
        additionalKm: initialAgreement?.additionalKm || 0,
        pricePerKm: initialAgreement?.pricePerKm || 0,
        additionalDays: initialAgreement?.additionalDays || 0,
        pricePerDay: initialAgreement?.pricePerDay || 0,
        damages: initialAgreement?.damages || 0,
        delayPayments: initialAgreement?.delayPayments || 0,
        otherCharges: initialAgreement?.otherCharges || 0,
        paidAmount: initialAgreement?.paidAmount || 0,
    },
  });
  ...
  }, [user, authLoading, bookingId, router, initialBooking, toast]);
